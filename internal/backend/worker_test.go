...
func TestWorkerSpeaksTheProtocol(t *testing.T) {
	...
	out, err := w.Step(windows, []float32{0.7, 0.9}, []uint64{1, 2})
	if err != nil {
		t.Fatalf("step: %v", err)
	}
...
func TestWorkerSurfacesAnErrorFrame(t *testing.T) {
	...
	_, err = w.Step([][]int32{oneWindow(1)}, []float32{0.7}, []uint64{1})
	if err == nil {
...
func TestWorkerChecksItsArgumentsBeforeTheWire(t *testing.T) {
	...
	for _, c := range cases {
		if _, err := w.Step(c.windows, c.temps, c.seeds); err == nil {
	...
	if _, err := w.Step([][]int32{oneWindow(5)}, []float32{0.7}, []uint64{1}); err != nil {
